...
public class QuizTotalCountInDashboard {
	...
	public void create_Form_Method() throws IOException {
	 ...
	 // Extent - Initialize Reporter
        ...
        ExtentTest test = extent.createTest("SurveyHeart - Quiz Dashboard Total Forms Count Test");
	  ...
	  // Fetching Total number of Quiz Count in Quiz Dashboard
	     try {
	    	    WebElement quizTotalCount = wait.until(ExpectedConditions.visibilityOfElementLocated(
	    	        By.xpath("//span[contains(text(),'(') and contains(text(),')')]")));    
	     test.info("Total Quiz count in quiz dashboard : " + quizTotalCount.getText());
	    	} catch (NoSuchElementException e) {
	     test.warning("Total quiz count element not found in dashboard");
	    	}
