...
public class AdminFormEditing {
	...
	public void create_Form_Method() throws IOException {
	 ...
	 // Extent - Initialize Reporter
        ...
        ExtentTest test = extent.createTest("SurveyHeart - Form Update by Admin Test");
	  ...
	  // Adding Long Question in Builder
	     ...
	     driver.findElement(By.xpath("//span[text()='Settings']")).click();
	     driver.findElement(By.xpath("//span[@class='icon-title'][normalize-space()='Submit']")).click();
	     test.pass("Admin has edited the shared form and added a new question : " + dynamicFormTitle);
	...
	/*  this is also correct if you want use - where it is need
	      // Wait for the question count element
	 
	     WebElement totalQuestionCount = wait.until(ExpectedConditions.visibilityOfElementLocated(
	         By.xpath("//span[@id='total_question_count']")));

	  // Get the text like "1 of 10" or similar
	     String totalQuestionsText = totalQuestionCount.getText();          // Example: "1 of 10"

	  // Extract only the numeric part after "of"
	     String totalOnlyDigits = "";
	     if (totalQuestionsText.contains("of")) {
	         String[] parts = totalQuestionsText.split("of");
	         if (parts.length == 2) {
	             totalOnlyDigits = parts[1].replaceAll("[^\\d]", "");  // remove non-digits
	         }
	     }

	  // Print to console and Extent Report
	     System.out.println("Total number of questions : " + totalOnlyDigits);
	     test.info("Total number of questions : " + totalOnlyDigits);
       */
