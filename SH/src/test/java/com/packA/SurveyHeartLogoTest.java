...
public class SurveyHeartLogoTest {
	
	
	@Test
	public void create_Form_Method() throws IOException, InterruptedException {
		
	 // Extent - Initialize Reporter
        ExtentSparkReporter spark = new ExtentSparkReporter("ExtentReports/FormReport.html");
        ExtentReports extent = new ExtentReports();
        extent.attachReporter(spark);
        ExtentTest test = extent.createTest("SurveyHeart - Logo Test");
	 ...
	 // Logo Fetching
	    wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@alt='survey-heart-icon']")));
	    WebElement logo = driver.findElement(By.xpath("//img[@alt='survey-heart-icon']"));
	    String logoSrc = logo.getAttribute("src");

	 // Log the logo with an embedded <img> tag in Extent Report
	    test.pass("SurveyHeart Logo is displayed below :<br><img src='" + logoSrc + "' width='100' height='100'>");
