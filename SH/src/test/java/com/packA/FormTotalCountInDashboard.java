...
public class FormTotalCountInDashboard {
	...
	public void create_Form_Method() throws IOException {
	 ...
	 // Extent - Initialize Reporter
        ...
        ExtentTest test = extent.createTest("SurveyHeart - Form Dashboard Total Forms Count Test");
	  ...
	  // Fetching Total number of Form Count in Form Dashboard
	     try {
	    	    WebElement formTotalCount = wait.until(ExpectedConditions.visibilityOfElementLocated(
	    	        By.xpath("//span[contains(text(),'(') and contains(text(),')')]")));    
	     test.info("Total Form count in form dashboard : " + formTotalCount.getText());
	    	} catch (NoSuchElementException e) {
	     test.warning("Total form count element not found in dashboard");
	    	}
