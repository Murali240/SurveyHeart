...
public class QuizLinkSendViaWhatsApp {
    ...
    public void form_Creation_With_Response_Method() throws IOException, InterruptedException {
     ...
     // Extent Report Setup
        ...
        ExtentTest test = extent.createTest("SurveyHeart - Send Quiz Link via WhatsApp Test");
    ...
    //  Login
     ...
     // Get Dynamic Quiz link
        WebElement quizLink = wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.xpath("//input[contains(@value,'https:')]")));

     // Fetch and log the actual quiz link
        String actualQuizLink = quizLink.getAttribute("value");
        test.pass("Quiz created with title : " + dynamicQuizTitle);
        test.info("Quiz Link : " + actualQuizLink);
     ...
     // Switch to new window
        ...
        try {
            ...
            try {
                ...
                System.out.println("WhatsApp dynamic popup is not handled automatically and closed manually only");
            }
         ...
         // Send the message
            ...
            test.pass("Quiz link has sent successfully, to WhatsApp contact : " + contactName);
