...
public class DuplicatingQuizTest {
	...
	public void create_Form_Method() throws IOException {
	 ...
	 // Extent - Initialize Reporter
        ...
        ExtentTest test = extent.createTest("SurveyHeart - Quiz Duplicating Test");
	  ...
	  // Identifying the created Form card in Form Dashboard
	     ...
	     driver.findElement(By.xpath("//p[normalize-space()='Duplicate Quiz']")).click();
	     
	  // Builder screen
	     WebElement builderScreen=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[@id='label_field_Title']//input[@type='text']")));
	     String duplicatedQuizTitle = builderScreen.getAttribute("value");
	     test.info("Quiz got duplicated with : "+duplicatedQuizTitle);
	     
	     WebElement questionTitle=wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='question-text-input-0']")));
	     String actualQuestionTitle = questionTitle.getAttribute("value");
	     String expectedQuestionTitle = "Short Question";
	     
	  // Assert that actualQuestionTitle and expectedQuestionTitle 
	     Assert.assertEquals(actualQuestionTitle, expectedQuestionTitle, "Question title does not match expected title");
	     test.pass("For duplicated Quiz, Question title is matched successfully");
	  ...
	  // Identifying the created Form card in Form Dashboard
	     String actualFormTitle = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='list-card-title-0']//p[@id='card-form-title']"))).getText();
	     String expectedFormTitle = duplicatedQuizTitle;
	     
	     
	  // Assert that actualTitle and expectedTitle 
	     Assert.assertEquals(actualFormTitle, expectedFormTitle, "Duplicated Quiz title is matched with the expected title");
	     test.pass("Duplicated Quiz title is matched with expected quiz title in Quiz dashboard");  
