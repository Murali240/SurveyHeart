...
public class SurveyHeartLogo {
	...
	public void create_Form_Method() throws IOException {
	 ...
	 // Extent - Initialize Reporter
	  ...
	  // Replace the XPath below with the correct one if needed
	     WebElement logo = wait.until(ExpectedConditions.visibilityOfElementLocated(
	         By.xpath("//img[@alt='survey-heart-icon']")   // SurveyHeart logo 
	     ));

	  // Validate if Logo is displayed using isDisplayed() method
	     if (logo.isDisplayed()) {
	         test.pass("SurveyHeart logo is displayed after login.");
	     } else {
	         test.fail("SurveyHeart logo is NOT displayed after login.");
	         Assert.fail("SurveyHeart logo is not visible.");
	     }
