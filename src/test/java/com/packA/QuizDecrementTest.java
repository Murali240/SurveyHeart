...
public class QuizDecrementTest {
    ...
    public void form_Creation_With_Response_Method() throws IOException, InterruptedException {
     // Extent - Initialize Reporter
        ...
        ExtentTest test = extent.createTest("SurveyHeart - Quiz Decrement Test");
     ...
     // Creating Quiz
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[text()='Quizzes']"))).click();
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[text()='Create Quiz']"))).click();
     ...
     // Step 1: Click the Upload area
        ...
        submitBtn.click();
        test.pass("Video file uploaded and quiz submitted successfully");

        
	  // Submitted page
	     WebElement successMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h3[text()='Submitted']")));
	     if (successMsg.isDisplayed()) {
	     test.pass("Quiz submitted and Thank you confirmation message got displayed");
	        }
	  ...
	  // if Follow Us popup then handle it
	     try {
	    	    ...
	    	    test.info("Follow Us popup did not appear this time and Continuing test execution.");
	    	}
	  ...
	  // User Account icon
	     ...
	     test.info(userCurrentPlan + " user Storage, before quiz decrement : " + currentPlanStorage);
	     ...
	     test.info(userCurrentPlan + " user Total Responses, before quiz decrement : " + currentPlanResponse);
	  ...
	  // Images
	     try {
	    	    ...
	    	    test.info(userCurrentPlan + " user Images, before quiz decrement : " + currentPlanImages);
	    	} catch (Exception e) {
	  ...
	  // Themes
	     try {
	    	    ...
	    	    test.info(userCurrentPlan + " user Themes, before quiz decrement : " + currentPlanThemes);
	    	} catch (Exception e) {
	  ...
	  // Identifying the created Form card in Quiz Dashboard
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[text()='Quizzes']"))).click();
	     ...
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@id='more0']"))).click();  // More for control page
	     driver.findElement(By.xpath("//p[@id='dashboard-view-responses']")).click();                        // View Responses
	     
	  // Overview screen
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[normalize-space()='Individual']"))).click();  // Individual button
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='Delete']"))).click(); // Delete button
	     driver.findElement(By.xpath("//label[normalize-space()='DELETE']")).click();                        // response deleted 
	  ...
	  // Identifying the created Form card in Quiz Dashboard
	     ...
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@id='more0']"))).click();    // More
	     driver.findElement(By.xpath("//div//p[text()='Edit Quiz']")).click();                                 // Edit Quiz

	  // IMAGE Delete
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@alt='attach']"))).click();   // Attach icon
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='popup-menu-container']//div//div[1]"))).click(); // Image option
	     wait.until(ExpectedConditions.visibilityOfElementLocated
	    	(By.xpath("//div[@class='builder-box1']//div[@id='default-theme-container']//div[2]//div[1]//img[1]"))).click(); // x Delete icon
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//label[normalize-space()='Delete']"))).click();     // Delete button on popup
	     wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='theme-close-button']//img"))).click();    // Close image folder
	     

	  // THEME UPLOAD
	     wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[text()='Settings']"))).click();
	     wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[@id='theme_card_undefined']//img[@class='theme-image']"))).click();  // Default theme 
	     wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[id='theme_card_0'] div div[class='theme-round center']"))).click();  // +My Themes button
	     wait.until(ExpectedConditions.visibilityOfElementLocated
	    	   (By.xpath("//body/div[@id='app']/div/div[@class='app-container']/div[@class='body-container']/div[@class='builder-wrapper']/div[@id='theme-container']/div[@id='default-theme-container']/div[2]/div[1]/img[1]"))).click();  // x delete icon
	     wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//label[normalize-space()='Delete']"))).click();  // Delete button
	     wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//img[@src='images/close_black.svg']"))).click(); // Closing theme folder
	  ...
	  // User Account icon
	     ...
	     String currentPlanStorage2 =  wait.until(ExpectedConditions.visibilityOfElementLocated(
	         By.xpath("//span[contains(text(),'of 1GB Used')]"))).getText();
	     test.info(userCurrentPlan + " user Storage, after quiz decrement : " + currentPlanStorage2);

	     String currentPlanResponse2 = wait.until(ExpectedConditions.visibilityOfElementLocated(
	         By.xpath("//span[contains(text(),'of 10000 Used')]"))).getText();
	     test.info(userCurrentPlan + " user Total Responses, after quiz  decrement : " + currentPlanResponse2);
	  ...
	  // Images
	     try {
	    	    ...
	    	    test.info(userCurrentPlan + " user Images, after quiz decrement : " + currentPlanImages2);
	    	} catch (Exception e) {
	    	    test.warning(" User Images usage info not visible or not present for this plan.");
	    	}  
	  ...
	  // Themes
	     try {
	    	    String currentPlanThemes2 = wait.until(ExpectedConditions.visibilityOfElementLocated(
	    	        By.xpath("//div[contains(text(),'Themes') or contains(.,'Themes')]/span[contains(text(),'of 50 Used')]")
	    	    )).getText();
	    	    test.info(userCurrentPlan + " user Themes, after quiz decrement : " + currentPlanThemes2);
	    	} catch (Exception e) {
	    	    test.warning("User Themes usage info not visible or not present for this plan.");
	    	}
