...
/** Test class to verify search functionality in tabular answers for a quiz */
@Listeners(com.surveyheart.listeners.TestListener.class)
public class QuizAnswersTabularSearchTest extends BaseTest {
	
	@Test 
	public void verifySearchResultsInTabularAnswerForQuiz() {
	 ...
	 // Quiz Dashboard - Initialize the Quiz Dashboard Page object with the current WebDriver instance
	    ...
	    QuizBuilderPage quizBuilderPage = new QuizBuilderPage(driver);
	             ...
	             // Set dynamic Quiz title
				    String dynamicQuizTitle = "QuizAnswerSearchInTabular " + System.currentTimeMillis();
	...
	// Quiz page - Initialize the Quiz page object with the current WebDriver instance
	   QuizPage quizPage = new QuizPage(driver);
					...
					ExtentManager.getTest().pass("First quiz answer submitted successfully for quiz: <b>" + quizTitle + "</b>");
	...
	// Submitted page - Initialize the Submitted page object with the current WebDriver instance
	  ...
	  // Answers page - Initialize the Overview page object with the current WebDriver instance
	     AnswersPage answersPage = new AnswersPage(driver);	
					  ...
					  answersPage.searchAnswerBar("Madhu Simma");        // just pass response text
					  ExtentManager.getTest().info("Searched answer text in tabular page: <b>Madhu Simma</b>");


				   // Assertion for searched reponse text vs expected response text
					  String searchedFirstAnswer = answersPage.getFirstSearchedAnswer().getText();
					  String expectedAnswerText = "Madhu Simma";
				   
					  Assert.assertEquals(searchedFirstAnswer, expectedAnswerText, "Searched quiz answer does not match expected answer text");
					  ExtentManager.getTest().pass("Searched quiz answer is matched with expected quiz answer: <b>"+ dynamicQuizTitle+"</b>");
