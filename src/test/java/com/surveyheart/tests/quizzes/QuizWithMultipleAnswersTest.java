...
/** Test class to verify that a quiz allows multiple attempts when 'Allow Multiple Attempts' is enabled */
@Listeners(com.surveyheart.listeners.TestListener.class)
public class QuizWithMultipleAnswersTest extends BaseTest {
	
	@Test (groups = "regression", priority = 21)
	public void verifyQuizWithMultipleAnswers() {
	 ...
	 // Quiz Dashboard - Initialize the Quiz Dashboard Page object with the current WebDriver instance
	    ...
	    QuizBuilderPage quizBuilderPage = new QuizBuilderPage(driver);
	             ...
	             // Set dynamic Quiz title
				    String dynamicQuizTitle = "MultipleAnswersQuiz " + System.currentTimeMillis();
	...
	// Submitted page - Initialize the Submitted page object with the current WebDriver instance
	   SubmittedPage submittedPage = new SubmittedPage(driver);
					...
					String totalQuestionCount2 = quizPage.getTotalQuestionCount();
					ExtentManager.getTest().info("Quiz page displayed with " + totalQuestionCount2 + " question."); 
					quizPage.clickSubmitButton();
