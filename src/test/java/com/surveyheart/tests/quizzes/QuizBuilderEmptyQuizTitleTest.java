...
/** Test class to verify that an error message is displayed when trying to submit a Quiz without a Quiz title */
@Listeners(com.surveyheart.listeners.TestListener.class)
public class QuizBuilderEmptyQuizTitleTest extends BaseTest {
	
	@Test 
	public void verifyErrorMessageForEmptyQuizTitle() {
	 ...
	 // Quiz Settings - Initialize the Quiz settings object with the current WebDriver instance
	   	QuizSettingsPage quizSettingsPage = new QuizSettingsPage(driver);
	   				...
	   				String actualMessage=quizSettingsPage.getQuizTitleRequiredMessage().getText();
					String expectedMessage="TITLE IS REQUIRED";

			     // Assert that Quiz title required warning popup 
				    Assert.assertEquals(actualMessage, expectedMessage, "Quiz title required does not displayed");
				    ExtentManager.getTest().pass("Captured error message for empty quiz title: <b>" + actualMessage + "</b>");
