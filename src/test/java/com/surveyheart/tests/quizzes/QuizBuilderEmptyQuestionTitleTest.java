...
/** Test class to verify that an error message is displayed when trying to submit a Quiz with an empty question title */
@Listeners(com.surveyheart.listeners.TestListener.class)
public class QuizBuilderEmptyQuestionTitleTest extends BaseTest {
	
	@Test 
	public void verifyErrorMessageForEmptyQuizQuestionTitle() {
	 ...
	 // Quiz Dashboard - Initialize the Quiz Dashboard Page object with the current WebDriver instance
	    ...
	    QuizBuilderPage quizBuilderPage = new QuizBuilderPage(driver);
	             ...
	             // Don't worry Quiz title
				    String dynamicQuizTitle = "QuizEmptyQuestionTitle " + System.currentTimeMillis();
	 ...
	 // Quiz Settings - Initialize the Quiz settings object with the current WebDriver instance
	   	QuizSettingsPage quizSettingsPage = new QuizSettingsPage(driver);
	   				...
	   				quizSettingsPage.clickQuizEmptyTitleSubmitButton();
	   		
	   				String actualMessage=quizSettingsPage.getQuestionTitleRequiredMessage().getText();
					String expectedMessage="INCOMPLETE QUESTIONS :1";

			     // Assert that Quiz title required warning popup 
				    Assert.assertEquals(actualMessage, expectedMessage, "Quiz question title required does not displayed");
				    ExtentManager.getTest().pass("Captured error message for empty quiz question title: <b>" + actualMessage + "</b>");
