...
/** Test class to verify that an Admin collaborator can edit a shared quiz by adding new questions */
@Listeners(com.surveyheart.listeners.TestListener.class)
public class AdminQuizEditingTest extends BaseTest {
	
	@Test
    public void verifyAdminQuizEditing()  {
	 ...
	 // ===== Quiz Builder Page =====
		QuizBuilderPage quizBuilderPage = new QuizBuilderPage(driver);
				     ...
				     // Set dynamic Quiz title
						String dynamicQuizTitle = "AdminQuizEditing " + System.currentTimeMillis();
	 ...
	 // Shared Quiz Dashboard - Initialize the Share popup object with the current WebDriver instance
		SharedQuizDashboardPage sharedQuizDashboard = new SharedQuizDashboardPage(driver);
			         ...
			         // Settings
					    quizSettingsPage.clickSettingsButton();
					    quizSettingsPage.clickSubmitButton();
			            ExtentManager.getTest().pass("Admin successfully edited the shared quiz and added a new question.");
	  ...
	  // Quiz page - Initialize the Form page object with the current WebDriver instance
		 QuizPage quizPage = new QuizPage(driver);
			         ...
			         // Quiz page
					    ...
					    ExtentManager.getTest().pass("Admin's newly added question was displayed on the Quiz page : <b>" + quizTitle+"</b>");
...
/**
Scenario: Test class to verify that an Admin collaborator can edit a shared quiz.

Steps:
1. Login as the Owner, create a new quiz with a dynamic title and a Short Answer question.
2. Add an Admin collaborator in Quiz Settings, then sign out.
3. Login as Admin, open the shared quiz, and add a new Long Answer question.
4. Save the updated quiz and sign out.
5. Login back as Owner, open the quiz, and verify that the new question appears on the Quiz page.

Expected Result:
- Admin can successfully edit the shared quiz and add new questions.
- Owner sees the updated quiz with the newly added question.
*/
