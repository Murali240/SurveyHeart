...
/** Verifies that the "Undo" toast message is displayed in the Form Individual page after deleting a response */
@Listeners(com.surveyheart.listeners.TestListener.class)
public class FormIndividualPageUndoToastMessageTest extends BaseTest {
	
	@Test 
    public void verifyUndoToastMessageDisplayedInFormOverview() {
	 ...
	 // Form Builder - Initialize the Form builder object with the current WebDriver instance
        FormBuilderPage builder = new FormBuilderPage(driver);
				     ...
				     // Enter Form title in Builder screen
				        String dynamicFormTitle = "UndoToastMessageInFormOverview " + System.currentTimeMillis();
...
/**
Scenario: Regression test to verify the "Undo" toast message is displayed in Form Individual page after deleting a response.
...
2. Open the form via Share popup, submit two responses, and close the form.
3. From the Form Dashboard, go to More Options → View Responses → Individual tab.
4. Delete one response and confirm the deletion.
5. Verify that the "Undo" toast message appears and that the total responses count is updated accordingly.

Expected Result:
- The "Undo" toast message is displayed successfully after deleting a response, and the response count is updated correctly.
*/
